"""
ZFJ HTML Parser + Timesheet Filler — Flask app (Vercel zero-config).

Vercel auto-detects a Flask instance named `app` in app.py at the repo root.
No vercel.json or `builds`/`routes` config needed.
"""
...
# Add localhost here for local frontend development.
ALLOWED_ORIGINS = {
    "https://managementkas-five.vercel.app",
    "http://localhost:3000",
    "http://localhost:5173",
}
...
def _cors_origin() -> str:
    """Return the caller's Origin if allowed, else the production origin."""
    origin = request.headers.get("Origin", "")
    if origin in ALLOWED_ORIGINS:
        return origin
    return "https://managementkas-five.vercel.app"


@app.before_request
def handle_preflight():
    """Answer OPTIONS preflight immediately with CORS headers."""
